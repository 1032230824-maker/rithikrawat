    ...
    <section id="services" className="py-32 px-4">
      <div className="max-w-6xl mx-auto">
        <AnimatedSection>
          ...
          <p className="font-inter text-center text-slate-500 mb-4 text-lg">Your End-to-End Content Partner for Visibility, Engagement & Revenue</p>
          <p className="font-inter text-center text-slate-400 mb-16 text-sm max-w-2xl mx-auto">Whether you're battling low engagement, struggling with inconsistent posting, or need cinematic reels that actually convert — I handle everything from shoot to strategy, so you can focus on running your business.</p>
        </AnimatedSection>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {services.map((service, index) => (
            <AnimatedSection key={index} delay={index * 0.08}>
              <motion.div
                className="group relative bg-white/80 backdrop-blur-sm rounded-2xl p-7 border border-slate-200 hover:border-transparent transition-all duration-500 hover:shadow-2xl overflow-hidden h-full"
              ...
              >
                ...
                <h3 className="font-satoshi text-xl font-bold mb-4 text-slate-700">{service.title}</h3>
                <ul className="space-y-3">
                  {service.items.map((item, i) => (
                    <li key={i} className="font-inter text-sm text-slate-500 flex items-start gap-2.5">
        ...
        <AnimatedSection>
          <div className="grid md:grid-cols-2 gap-6">
            ...
            >
              ...
              <h3 className="font-satoshi text-2xl font-bold mb-3 text-slate-700">PERSONAL BRAND ACCELERATOR</h3>
              <p className="font-inter text-sm text-slate-500 mb-4">Position yourself as the undeniable authority in your space with content that builds credibility and attracts opportunities.</p>
            ...
            <motion.div
              className="bg-gradient-to-br from-cyan-500 to-blue-600 rounded-2xl p-9 text-white flex flex-col justify-between relative overflow-hidden"
              whileHover={{ y: -4, transition: { duration: 0.3 } }}
            >
              <div className="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2"></div>
              <div className="absolute bottom-0 left-0 w-24 h-24 bg-white/5 rounded-full translate-y-1/2 -translate-x-1/2"></div>
              <div className="relative z-10">
                <h3 className="font-satoshi text-3xl font-bold mb-4">FULL-SERVICE CONTENT PARTNERSHIP</h3>
              ...
              <div className="relative z-10">
                <p className="font-inter text-sm opacity-80 mb-4">Investment starting at <span className="text-2xl font-bold font-satoshi">₹15k</span>/month</p>
                <Button
                  className="bg-white text-blue-600 hover:bg-white/90 font-inter font-semibold rounded-full px-8 py-5 hover:shadow-lg transition-all duration-300 hover:scale-105"
